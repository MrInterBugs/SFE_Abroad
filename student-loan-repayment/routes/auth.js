...
function loginMessage(req) {
  if (req.query.registered) return 'Account created. Check your email to confirm your address before signing in.';
...
router.post('/register', authRateLimit, verifyCsrfToken, async (req, res) => {
  ...
  try {
    ...
    await sendEmailConfirmation(email.toLowerCase(), token);
